...
// === НОВАЯ ЛОГИКА ДЛЯ РАЗДЕЛА ТУРНИРОВ ===
...
function renderFilteredTournaments() {
    ...
    if (filtered.length > 0) {
        filtered.forEach(tournament => {
            ...
            if (isArchive) {
                tournamentCard.classList.add('archived');
            }
            ...
            tournamentCard.innerHTML = `
              ...
              <div class="tournament-info">
                ...
                <span>${tournament.scale}</span>
                <span>Дата: ${tournament.date}</span>
                <span>Город: ${tournament.city}</span>
              </div>
            `;
...
async function showTournamentDetails(tournamentId) {
  try {
    ...
    header.innerHTML = `
      ...
      <p>Дата: ${data.date}</p>
      <p>Масштаб: ${data.scale || 'Не указан'}</p>
...
async function loadTournamentPosts(tournamentId) {
  ...
  try {
    ...
    if (postsList) postsList.innerHTML = '<p>Ошибка загрузки постов.</p>';
  }
}
...
// ... (весь код до showTournamentDetails остается без изменений) ...

async function showTournamentDetails(tournamentId) {
  try {
    const [tournamentData, registrationsData] = await Promise.all([
        supabaseFetch(`tournaments?id=eq.${tournamentId}&select=*`, 'GET'),
        supabaseFetch(`registrations?tournament_id=eq.${tournamentId}&select=id,status,speaker1_username,speaker2_username`, 'GET')
    ]);
    ...
    // Отрисовка шапки турнира
    const header = document.getElementById('tournament-header');
    header.innerHTML = `
      <img src="${tournament.logo || 'placeholder.png'}" alt="Логотип турнира" onerror="this.src='placeholder.png'">
      <strong>${tournament.name}</strong>
    ...
    let activeTabId = 'tournament-posts'; // Вкладка по умолчанию

    // Посты
    loadTournamentPosts(tournamentId);

    // Регистрация (видна всем)
    ...
    if (isCreator) {
        // Вкладка ТЭБ (только для создателя)
        ...
        loadTabManagement(tournamentId, registrationsData);
    } else if (tournament.tab_published) {
        // Вкладка Участники (для всех, если ТЭБ опубликован)
    ...
    tabsContainer.addEventListener('click', (e) => {
        if (e.target.classList.contains('tab-btn')) {
            ...
            activeTabId = `tournament-${tabId.replace('management', 'tab-management')}`;
...
async function updateRegistrationStatus(registrationId, newStatus, isCreator) {
    try {
        await supabaseFetch(`registrations?id=eq.${registrationId}`, 'PATCH', { status: newStatus });
        
        // Обновляем данные на лету
        const registrationsData = await supabaseFetch(`registrations?tournament_id=eq.${currentTournamentId}&select=id,status,speaker1_username,speaker2_username`, 'GET');
        loadRegistrations(currentTournamentId, isCreator);
        loadTabManagement(currentTournamentId, registrationsData);
        ...
        registrations.forEach(reg => {
            ...
            if (isCreator) {
                ...
                if (reg.status === 'pending') {
                    ...
                    actionsHtml += `<button class="action-btn remove" data-id="${reg.id}" data-status="pending">❌ Убрать из ТЭБа</button>`;
                }
                actionsHtml += '</div>';
            }
        ...
        if (isCreator) {
            registrationList.querySelectorAll('.action-btn').forEach(button => {
                button.addEventListener('click', (e) => {
                    ...
                    updateRegistrationStatus(regId, newStatus, isCreator);
                });
            });
        }
    } catch (error) {
...
async function loadTabManagement(tournamentId, registrations) {
    ...
    const { data: tournamentData } = await supabaseFetch(`tournaments?id=eq.${tournamentId}&select=tab_published`, 'GET');
    const isPublished = tournamentData[0].tab_published;
    ...
    const renderList = (list, teams) => {
        ...
        teams.forEach(reg => {
            ...
            actionsHtml += `<button class="action-btn remove" data-id="${reg.id}" data-status="pending">❌ Убрать</button>`;
            actionsHtml += '</div>';

            card.innerHTML = `
                <div class="registration-card-header"><strong>${reg.faction_name}</strong></div>
    ...
    document.querySelectorAll('#tournament-tab-management .action-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const regId = e.target.dataset.id;
            const newStatus = e.target.dataset.status;
            updateRegistrationStatus(regId, newStatus, true);
        });
    });
}
...
async function loadParticipants(tournamentId) {
    // Функция для отображения ТЭБа обычным пользователям
    ...
    // ... (аналогично loadTabManagement, но без кнопок управления)
}

// ... остальной код (initBracket, loadBracket, и т.д.)

// Убедитесь, что все остальные функции, которые были в файле, остаются на месте
